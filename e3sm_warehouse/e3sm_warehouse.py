"""Main e3sm_warehouse module."""
import os
from re import I
import sys

import ipdb
from e3sm_warehouse.dataset import DatasetStatusMessage
import yaml
import inspect
import fnmatch
...
from e3sm_warehouse.slurm import Slurm
from e3sm_warehouse.listener import Listener
import e3sm_warehouse.resources as resources
import e3sm_warehouse.util as util
...
class AutoWarehouse:
    def __init__(self, *args, **kwargs):
        ...
        setup_logging("debug", f"{self.slurm_path}/e3sm_warehouse.log")
        ...
        if self.serial is True:
            log_message("info", "Running e3sm_warehouse in serial mode")
        else:
            log_message(
                "info",
                f"Running e3sm_warehouse in parallel mode with {self.num_workers} workers",
            )
    ...
    def find_e3sm_source_dataset(self, job):
        ...
        """
        # msg = f"No raw E3SM dataset was in the list of datasets provided, seaching the e3sm_warehouse for one that mathes {job}"
    ...
    def setup_datasets(self, check_esgf=True):
        log_message("info", "WH: setup_datasets: Initializing the e3sm_warehouse")
    ...
    def start_datasets(self, datasets=None):
        ...
        for dataset_id, dataset in datasets.items():
            ...
            # for all the datasets, if they're not yet published or in the e3sm_warehouse
            ...
            for item in self.workflow.next_state(dataset, state, params):
                ...
                # written to the dataset status file
                    ...
                    msg = f"e3sm_warehouse: start_datasets: Dataset {dataset.dataset_id} transitioning to state {new_state}"
                    if params:
    ...
    def add_args(
        ...
        p = parser.add_parser(name=NAME, help="Automated e3sm_warehouse processing")
        p.add_argument(
        ...
        p.add_argument(
            "--e3sm_warehouse-config",
            default=DEFAULT_CONF_PATH,
            help="The default e3sm_warehouse/publication/archives paths are drawn from a config yaml file "
        ...
        p.add_argument(
            "--testing", action="store_true", help="run the e3sm_warehouse in testing mode"
        )
        p.add_argument(
            "--slurm-path",
            required=False,
