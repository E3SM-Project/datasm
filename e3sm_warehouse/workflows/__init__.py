...
from e3sm_warehouse.workflows import jobs
...
class Workflow(object):
    ...
    def load_jobs(self):
        ...
        for file in jobs_path.glob('*'):
            if file.name == '__init__.py' or file.is_dir():
                continue
            module_string = f'e3sm_warehouse.workflows.jobs.{file.stem}'
    ...
    def load_children(self):
        ...
        for d in os.scandir(my_path):
            ...
            workflows_string = f"e3sm_warehouse{os.sep}workflows"
            idx = str(my_path.resolve()).find(workflows_string)
            if self.name == NAME:
                module_name = f'e3sm_warehouse.workflows.{d.name}'
            else:
                module_name = f'e3sm_warehouse.workflows{str(my_path)[idx+len(workflows_string):].replace(os.sep, ".")}.{d.name}'
    ...
    def add_args(parser):
        parser.add_argument(
            ...
            help="Dataset IDs that should have the workflow applied to them. If this is "
                 "given without the data-path, the default e3sm_warehouse value will be used."
    ...
    def arg_checker(args, command=NAME):
        ...
        if isinstance(args.dataset_id, list) and len(args.dataset_id) > 1 and args.data_path:
            log_message('error', "\nMultiple datasets were given along with the --data-path. For multiple datasets you must use the --e3sm_warehouse-path and the E3SM publication directory structure")
            return False, command
        return True, command
