...
import os
import stat
from e3sm_warehouse.util import print_debug
...
class Slurm(object):
    ...
    def __init__(self):
        ...
        """
        if not any(
            os.access(os.path.join(path, "sinfo"), os.X_OK)
            for path in os.environ["PATH"].split(os.pathsep)
        ):
            raise Exception("Unable to find slurm, is it installed on this system?")
        ...
        for item in out.split("\n"):
            for j in item.split(" "):
                ...
                if attribute is None:
                    continue
                job_info.set_attr(attr=attribute, val=j[index + 1 :])
        return job_info
