...
gv_WH_root = '/p/user_pub/e3sm/e3sm_warehouse'
...
helptext = '''
    ...
    The default jobset config file is /p/user_pub/e3sm/archive/.cfg/jobset_config.  It contains
        ...
        pub_root=<path>         (default is /p/user_pub/e3sm/e3sm_warehouse/E3SM)
...
# return path to unique status file (e3sm_warehouse or publication)
# create in e3sm_warehouse if not found
...
# Must ensure we have a DSID name for the dataset status file, before e3sm_warehouse facet directory exists.
...
def main():
    ...
    while True:
        ...
        for am_spec_line in dataset_spec:
            ...
            ens_path = get_warehouse_path_via_dsid(dsid)        # intended e3sm_warehouse dataset ensemble path
            ...
            # BECOME TRANSFER:  move Holodeck files to e3sm_warehouse destination path:

            logMessage('INFO',f'ARCHIVE_EXTRACTION_SERVICE:Begin file transfer to e3sm_warehouse: {dest_path}')
            ...
            logMessage('INFO',f'ARCHIVE_EXTRACTION_SERVICE:Completed file transfer to e3sm_warehouse: filecount = {fcount}, ET = {ET}')
