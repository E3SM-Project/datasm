...
from e3sm_warehouse.util import (
    load_file_lines,
    search_esgf,
...
class Dataset(object):
    ...
    def latest_warehouse_dir(self):
        if self.warehouse_path is None or (
            ...
            log_message(
                "error", f"The dataset {self.dataset_id} does not have a e3sm_warehouse path"
            )
            sys.exit(1)
        ...
        # we assume that the e3sm_warehouse directory contains only directories named "v0.#" or "v#"
        try:
    ...
    def warehouse_version(self):
        """
        Returns the latest version number in the e3sm_warehouse directory. If not version exists
        then it returns 0
        """
    ...
    def get_status_from_pub_dir(self):
        ...
        if is_complete:
            ...
            # otherwise the "official" location should be the e3sm_warehouse
