...
if [ -z $arch_path ]; then
    ...
    native_case_id=`echo $first_file | cut -f1-3 -d.`
else
    native_case_id=`basename $arch_path`
fi
...
echo "    Native Case ID:         $native_case_id" >> $Report_Name
echo "    Case ID:                $case_id" >> $Report_Name
