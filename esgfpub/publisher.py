...
def publish_maps(mapfiles, ini, mapsin, mapsout, mapserr, username, password, debug=False):
    for m in mapfiles:
        ...
        cmd = 'myproxy-logon -s esgf-node.llnl.gov -l {} -t 72 -o ~/.globus/certificate-file'.format(username)
        if debug:
...
""".format(
        ...
        if debug:
            print_message('Running publication script: {}'.format(tempfile.name), 'info')
        ...
        try:
            ...
            try:
                ...
                print_message("Error in publication, moving {} to {}".format(m, mapserr))
                os.rename(
                ...
                if debug:
                    print_message("Publication success, moving {} to ".format(m, mapsout), "info")
                os.rename(
                    os.path.join(mapsin, m),
    ...
    if loop:
        ...
        while True:
            ...
            if not mapfiles:
                ...
                publish_maps(mapfiles, ini, mapsin, mapsout, mapserr, username, password, debug)
    else:
        mapfiles = os.listdir(mapsin)
        publish_maps(mapfiles, ini, mapsin, mapsout, mapserr, username, password, debug)
    return 0
