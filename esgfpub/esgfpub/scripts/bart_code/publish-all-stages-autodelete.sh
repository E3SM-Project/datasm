...
do
	#check for deletion needed

	telltale=`echo $mapfile | rev | cut -f1-2 -d. | rev`
	vnum=`echo $telltale | cut -f1 -d. | cut -c2-`
	if [ $vnum -gt 1 ]; then
		vpre=$(($vnum - 1))
		echo "version is $vnum, deleting version $vpre"
		newtail="#$vpre"
		echo $mapfile > tmp_mystring
		sedcmd=`echo "s/.$telltale/$newtail/g"`
		sed -i $sedcmd tmp_mystring
		to_delete=`cat tmp_mystring`
		echo "$ts: INPROCESS: Calling esgunpublish for $to_delete" >> $logdir/$rlog 2>&1
		esgunpublish --project e3sm --database-delete --delete $to_delete >> $logdir/$rlog 2>&1
		ts=`date +%Y%m%d.%H%M%S`
		echo "$ts: COMPLETED: Deletion of dataset $to_delete" >> $logdir/$rlog 2>&1
	fi
